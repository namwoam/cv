...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    videowriter = cv2.VideoWriter(
        "output2.avi", fourcc, film_fps, (film_w, film_h))
    ...
    while (video.isOpened()):
        ...
        if ret:  # check whethere the frame is legal, i.e., there still exists a frame
            ...
            marker_corners, ids, rejected = aruco.detectMarkers(
                frame, arucoDict, parameters=arucoParameters)
            corners = np.round(
                marker_corners[0].reshape((4, 2))).astype(np.int32)
            ...
            warping(ref_image, frame, H, np.min(corners, axis=0)[1], np.max(corners, axis=0)[
                    1], np.min(corners, axis=0)[0], np.max(corners, axis=0)[0])
